...
<!--
  ...
  Props:
    ...
    title      String       Kopf (Fallback / #title-Slot)
  ...
  Emits: update:collapsed, update:width
-->
<script setup>
...
const emit = defineEmits(["update:collapsed", "update:width"]);
...
onBeforeUnmount(() => { endResize(); window.removeEventListener("keydown", onKeydown); });
...
const headTitle = computed(() => (props.spec ? specName.value : props.title));
</script>
...
<template>
  ...
  <aside class="pp-inspector"
         ...
         :aria-label="overlay ? headTitle : null" :aria-hidden="overlay && collapsed ? 'true' : null"
    ...
    <template v-else>
      <div v-if="header" class="pp-inspector__head">
        <span class="pp-inspector__title"><slot name="title">{{ headTitle }}</slot></span>
      ...
      <div class="pp-inspector__body">
        <slot>
          <!-- eingebautes Spezifikations-Panel -->
          <template v-if="spec">
...
.pp-inspector__head { display: flex; align-items: center; justify-content: space-between; gap: var(--pp-space-2);
...
.pp-inspector__title { font-size: var(--pp-fs-13, 13px); font-weight: var(--pp-weight-semibold);
